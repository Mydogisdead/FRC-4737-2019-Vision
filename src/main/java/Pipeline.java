...
public class Pipeline {
    ...
    public static void gridDetection(Mat img) {
        ...
        Mat grid = Pipeline.orderPoints(approx);

        Pipeline.approx = approx;
        Pipeline.grid = grid;
        Pipeline.rect = rect;
    }
    ...
    public static Mat normalProcess(Mat img) {
        Mat threshImg = Pipeline.InvertImageColor(img);
        Pipeline.gridDetection(threshImg);
        ...
        mat = Imgproc.getPerspectiveTransform(Pipeline.grid, mat);

        Mat M = new Mat();

        Imgproc.warpPerspective(threshImg, M, mat, new Size(512, 512));
        return Pipeline.InvertImageColor(M);
    }

    public static Mat adaptativeProcess(Mat img) {
        Mat im = new Mat();
        Imgproc.threshold(img, im, 120, 255, Imgproc.THRESH_TRUNC);
        im = Pipeline.adaptativeThresholding(im);
        Imgproc.medianBlur(im, im, 7);
        Mat threshImg = Pipeline.InvertImageColor(im);
        Pipeline.gridDetection(threshImg);
